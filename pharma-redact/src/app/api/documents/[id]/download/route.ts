    ...
    try {
        ...
        if (document.status !== 'redacted' || !document.redactedFilePath) {
            return NextResponse.json(
                { error: 'Redacted version not available' },
                { status: 400 }
            );
        }
        ...
        if (!fs.existsSync(document.redactedFilePath)) {
            return NextResponse.json(
                { error: 'Redacted file not found' },
                { status: 404 }
            );
        }

        // Read the file
        const fileBuffer = fs.readFileSync(document.redactedFilePath);
        ...
        console.error('Error downloading redacted document:', error);
        return NextResponse.json(
